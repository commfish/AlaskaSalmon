...
profile <-function(i,z,xa.start, xa.end,lnalpha.c, beta){ 
  ...
  f.empty <- function(x){
    data.frame(S0 = rep(0, length(x[,1])))
  }
  f.empty1 <- function(x){
    data.frame(S0 = rep(1, length(x[,1])))
  }
  ...
  summarise_each(dat9, funs(median, 
                            q95=quantile(., 0.95, na.rm=T), 
                            q90=quantile(., 0.90, na.rm=T),
                            q10=quantile(., 0.10, na.rm=T),
                            q5=quantile(., 0.05, na.rm=T))) -> mq
  
  names(mq) <- c(rep(('Median'),length(x)+1), rep(('q95'),length(x)+1), 
                 rep(('q90'),length(x)+1), rep(('q10'),length(x)+1), rep(('q5'),length(x)+1))
  ...
  Y %>% 
    dplyr::select(Escapement, Optimal_Yield0.7 = oy_0.7, Overfishing0.7 = of_0.7, Optimal_Recruitment0.7 = or_0.7) %>% 
    melt(., id.vars = 'Escapement') %>% 
    ggplot(mY, aes(Escapement/1000, value, lty=variable))+geom_line()+
  ...
  ggsave("figures/0.7.AR.png", dpi=200, dev='png', width=8, height=5, units='in')
  

  Y %>% 
    dplyr::select(Escapement, Optimal_Yield0.8 = oy_0.8, Overfishing0.8 = of_0.8, Optimal_Recruitment0.8 = or_0.8) %>% 
    melt(., id.vars = 'Escapement') %>% 
    ggplot(mY, aes(Escapement/1000, value, lty=variable))+geom_line()+
    xlab('Escapement / 1,000')+ylab('Probability')+
    theme(legend.justification=c(1,0), legend.position=c(1,.5), legend.key = element_blank(),legend.title=element_blank())
  
  ggsave("figures/0.8.AR.png", dpi=200, dev='png', width=8, height=5, units='in')
  
  
  Y %>% 
    dplyr::select(Escapement, Optimal_Yield0.9 = oy_0.9, Overfishing0.9 = of_0.9, Optimal_Recruitment0.9 = or_0.9) %>% 
    melt(., id.vars = 'Escapement')  %>% 
  ...
  ggsave("figures/0.8_0.7.AR.png", dpi=200, dev='png', width=7, height=6, units='in')
  theme_set(theme_bw()+theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()))
  options(scipen=99999)
  
  QM <- read.csv("data/processed/QM.AR.csv")
